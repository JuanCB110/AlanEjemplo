  ...
  try {
    ...
    agregar.addEventListener('click', async function(){
      ...
      if(cantidad > 0){
        ...
        Swal.fire({
          title: 'Producto Agregado a la Cesta Exitosamente!',
          icon: 'success',
          showConfirmButton: true,
          confirmButtonText: 'Seguir comprando',
          showCancelButton: true,
          cancelButtonText: 'Ir a la cesta'
      }).then((result) => {
          if (result.isConfirmed) {
              // Acción si se pulsa "Seguir comprando"
              // Puedes dejar esto vacío si no necesitas realizar ninguna acción
              window.location.reload();
          ...
          comprar.addEventListener('click', async function(){
            ...
            if(cantidad > 0){
              Swal.fire({
                ...
                icon: 'success',
                // iconColor: '#9F79FF', // Cambia el color del ícono de éxito
                showConfirmButton: true, // Evita que se muestre el botón de confirmación
                // timer: 3000
            });
...
async function checkCart() {
  const cesElement = document.getElementById('ces');
  const cestaCollection = collection(firestore, 'cesta');

  try {
      const querySnapshot = await getDocs(cestaCollection);
      if (querySnapshot.size > 0) {
          cesElement.innerHTML = '<span class="material-symbols-outlined">production_quantity_limits</span>';
      }
  } catch (error) {
